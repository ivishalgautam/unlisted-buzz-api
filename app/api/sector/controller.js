...
const create = async (req, res) => {
  ...
  try {
    // const validateSectorData = SectorSchema.parse(req.body);
    let slug = slugify(req.body.name, { lower: true });
    req.body.slug = slug;

    const data = await table.SectorModel.create(req, { transaction });
...
const updateById = async (req, res) => {
  ...
  try {
    ...
    const record = await table.SectorModel.getByPk(req);
    if (!record) {
      return res
        .code(NOT_FOUND)
    ...
    if (req.body.name) {
      const slugExist = await table.SectorModel.getBySlug(req, req.body.slug);
      ...
      if (slugExist && record?.id !== slugExist?.id)
        return res.code(BAD_REQUEST).send({
          status: false,
          message: "Sector exist with this name!",
        });
    }
...
const getBySlug = async (req, res) => {
  try {
    const record = await table.SectorModel.getBySlug(req);
    if (!record) {
      return res
        .code(NOT_FOUND)
        .send({ status: false, message: "Sector not found!" });
    ...
    res.send({
      status: true,
      data: await table.SectorModel.getBySlug(req),
    });
  } catch (error) {
    throw error;
  }
...
const deleteById = async (req, res) => {
  ...
  try {
    ...
    const isSectorDeleted = await table.SectorModel.deleteById(
      req,
      req.params.id,
      {
        transaction,
    ...
    await transaction.commit();
    res.send({ status: true, message: "Sector deleted." });
  } catch (error) {
    await transaction.rollback();
    throw error;
  }
